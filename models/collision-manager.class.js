...
ifDeadIfCollidingMethod(enemy, bottle) {
  ...
  const hits =
    enemy instanceof EndbossLevel1
      ? bottle.isCollidingTight(enemy, 35) 
...
characterColliding(enemy) {
  ...
  if (isStomp) {return this.ifIsStompMethod(enemy, c);}
...
isBodyCollision(c, enemy) {
  ...
  return this.allInsetsMethod(c, enemy);
}

 /**
 * Performs collision detection between the character and an enemy
 * using reduced hitboxes for both entities to achieve precise and fair contact.
 */
  allInsetsMethod(c, enemy) {
    const { ax2, ax1, ay2, ay1 } = this.CharakterHitboxMethod(c);
    let { insetEnemyXFactor, insetEnemyYFactor } = this.insetEnemyFactorMethod(enemy);
    const { bx1, bx2, by1, by2 } = this.insetEnemyMethod(enemy, insetEnemyXFactor, insetEnemyYFactor);
    return ax2 > bx1 && ax1 < bx2 && ay2 > by1 && ay1 < by2;
  }

 /**
 * Calculates a reduced enemy hitbox based on horizontal and vertical
 * inset factors relative to the enemy's dimensions.
 */
  insetEnemyMethod(enemy, insetEnemyXFactor, insetEnemyYFactor) {
 ...
 /**
 * Determines enemy-specific inset factors for collision detection,
 * allowing different enemy types to have individually tuned hitboxes.
 */
  insetEnemyFactorMethod(enemy) {
    ...
    return { insetEnemyXFactor, insetEnemyYFactor };
  }

 /**
 * Calculates a reduced hitbox for the character to avoid premature
 * collision detection and improve overall gameplay feel.
 */
  CharakterHitboxMethod(c) {
