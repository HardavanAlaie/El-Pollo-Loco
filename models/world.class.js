...
class World {
  ...
  setWorld() {
    this.character.world = this;
    this.endboss = this.level.enemies.find((e) => e instanceof EndbossLevel1);
    this.spawnEnemyLoop();
  }
  ...
  checkCollisions() {
    (this.level.enemies || []).forEach((enemy) => {
      if (enemy instanceof EndbossLevel1) return; // Endboss wird separat behandelt
  ...
  checkEndbossDefeated() {
    if (!this.endboss || this.endbossDefeated || this.playerDied) return;
  ...
  // Die Methoden wie `draw`, `endGame`, `loadNextLevel`, `showLevelMessage`, `spawnEnemyLoop`, etc.
  // sind nicht verändert worden – falls du sie willst, kann ich sie auch einbauen.
}
